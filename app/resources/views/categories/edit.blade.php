...
@section('content')
    ...
    <div class="card card-app">
        <div class="card-body p-4">
            {{-- Integrar con: route('categories.update', $category) --}}
            <form action="{{ url('/categories/1') }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')
                <div class="row g-3 g-md-4">
                    <div class="col-12 col-md-6">
                        <label for="name" class="form-label fw-medium">Nombre <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="name" name="name" value="Granos" required>
                        <div class="invalid-feedback">El nombre es obligatorio.</div>
                    </div>
                    <div class="col-12 col-md-6">
                        <label for="description" class="form-label fw-medium">Descripción</label>
                        <textarea class="form-control" id="description" name="description" rows="4">Arroz, frijoles, lentejas y cereales.</textarea>
                    </div>
                </div>
